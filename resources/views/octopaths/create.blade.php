...
@section('content')
    ...
    {!! Form::model($octopath_dataset, ['route'=>'octopaths.store']) !!}
        ...
        {!! Form::text('octopath_title', old('octopath_title')) !!}
        <br>
        ...
        @for($i=0; $i<$merge_num; $i++)
            ...
            {!! Form::text( 'title'. ($i+1), old('title'. ($i+1)) ) !!}
            <br>
            
            {{-- link form --}}
            {!! Form::label('link'. ($i+1), 'Link'. ($i+1)) !!}
            {!! Form::text( 'link'. ($i+1), old('link'. ($i+1)) ) !!}
            <br>
            
            {{-- description form --}}
            {!! Form::label('description'. ($i+1), 'Description'. ($i+1)) !!}
            {!! Form::text( 'description'. ($i+1), old('description'. ($i+1)) ) !!}
            <br>
    ...
    <a href="/create?merge_num={{ $merge_num_plus_one }}"><button class="btn btn-success">+ link</button></a>
    <a href="/create?merge_num={{ $merge_num_minus_one }}"><button class="btn btn-danger">- link</button></a>
@endsection